...
function isOfficePreviewable(fileType?: string, fileName?: string) {
  const normalizedType = fileType?.toLowerCase() || '';
...
export default function FacultyNoteDetailScreen() {
  ...
  const officePreviewUrl =
    isOfficePreviewable(selectedAttachment?.fileType, selectedAttachment?.fileName)
      ? getOfficePreviewUrl(selectedAttachmentUrl)
      : null;

  const handleOpenExternally = async () => {
    if (!selectedAttachmentUrl) {
      Alert.alert('File unavailable', 'This attachment does not have an openable file URL yet.');
      return;
    }
        ...
        <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.heroCard}>
            <View style={styles.heroTopRow}>
              <Pressable onPress={() => router.back()} style={styles.topButton}>
                <MaterialIcons name="arrow-back" size={18} color="#F7FBF9" />
                <ThemedText style={styles.topButtonText}>Back</ThemedText>
              </Pressable>
              <Pressable onPress={() => router.replace('/faculty-notes')} style={styles.topButton}>
                <MaterialIcons name="menu-book" size={18} color="#F7FBF9" />
                <ThemedText style={styles.topButtonText}>Notes</ThemedText>
              </Pressable>
            </View>

            <View style={styles.heroIconWrap}>
              <MaterialIcons name={getFileIcon(selectedAttachment?.fileType)} size={30} color="#F7FBF9" />
            </View>
            <ThemedText type="title" style={styles.heroTitle}>
              {note?.title || 'Uploaded note'}
            </ThemedText>
            <ThemedText style={styles.heroSubtitle}>
              Review the note details and preview supported attachments right here in the module.
            </ThemedText>
          </View>
          ...
          {isLoading ? (
            <View style={styles.stateCard}>
              ...
              <ThemedText style={styles.stateText}>Fetching the uploaded details from Atlas.</ThemedText>
            </View>
          ) : errorMessage ? (
            <View style={styles.stateCard}>
              <MaterialIcons name="error-outline" size={26} color="#12634B" />
              <ThemedText style={styles.stateTitle}>Unable to load note</ThemedText>
            ...
            <>
              <View style={styles.panel}>
                <ThemedText style={styles.panelLabel}>Subject</ThemedText>
                <ThemedText style={styles.panelTitle}>
                  {subject?.subjectName || note?.subjectName || 'Subject not available'}
                </ThemedText>
                ...
                <View style={styles.metricsRow}>
                  <View style={styles.metricCard}>
                    <ThemedText style={styles.metricValue}>{formatUploadedAt(note?.uploadedAt)}</ThemedText>
                    <ThemedText style={styles.metricLabel}>Uploaded on</ThemedText>
                  </View>
                  <View style={styles.metricCard}>
              ...
              <View style={styles.detailCard}>
                ...
                {attachments.length ? (
                  ...
                  <View style={styles.stateCardCompact}>
                    <ThemedText style={styles.stateText}>No attachment was found for this note.</ThemedText>
                  </View>
                )}
              </View>
              ...
              <View style={styles.detailCard}>
                ...
                {!selectedAttachment ? (
                  ...
                  <View style={styles.previewFallback}>
                    <MaterialIcons name="picture-as-pdf" size={24} color="#12634B" />
                    ...
                    <Pressable onPress={() => void handleOpenPdfInBrowser()} style={styles.externalOpenButton}>
                      <MaterialIcons name="open-in-new" size={18} color="#F7FBF9" />
                      <ThemedText style={styles.externalOpenButtonText}>Open PDF</ThemedText>
                    </Pressable>
                  </View>
                  ...
                  <View style={styles.officePreviewWrap}>
                    <HtmlFrame
                      ...
                      style={styles.officeFrame}
                      title={selectedAttachment?.fileName || 'Office preview'}
                    />
                  </View>
                ) : (
                  <View style={styles.previewFallback}>
                    <MaterialIcons name="info-outline" size={24} color="#12634B" />
                    <ThemedText style={styles.previewFallbackTitle}>
                      {isOfficePreviewable(selectedAttachment?.fileType, selectedAttachment?.fileName)
                        ? 'Office file preview is limited here'
                        : 'Inline preview not available'}
                    </ThemedText>
                    <ThemedText style={styles.previewFallbackText}>
                      {isOfficePreviewable(selectedAttachment?.fileType, selectedAttachment?.fileName)
                        ? 'DOC, DOCX, XLS, XLSX, PPT, and PPTX files usually need either a deployed public file URL or a dedicated viewer app. During local development, a private localhost or LAN URL often makes the browser download the file instead of previewing it.'
                        : 'Images and PDFs can be shown directly here. Other formats usually need a dedicated viewer or conversion step, especially while the app is running on a local server.'}
                    </ThemedText>
                    <Pressable onPress={() => void handleOpenExternally()} style={styles.externalOpenButton}>
                      <MaterialIcons name="open-in-new" size={18} color="#F7FBF9" />
                      <ThemedText style={styles.externalOpenButtonText}>Open externally</ThemedText>
                    </Pressable>
                  </View>
                )}
              </View>
